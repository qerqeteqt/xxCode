...
> 想看**当前进度和完整功能清单**（包含工作流、工具、上下文、记忆、子 Agent 的现状、
> 以及所有已知局限），见 [`docs/STATUS.md`](docs/STATUS.md)。那是一份会过期的快照，
> 和代码对不上时以代码为准。
...
| | `MemoryExtractor` | `AutoDream` |
|---|---|---|
| 触发 | **每一轮对话结束后** | 攒够 `CONSOLIDATE_MIN_HOURS` + `MIN_SESSIONS` |
| 职责 | **提取** —— 把刚出现的事实写下来 | **整理** —— 合并、去重、更新过时 |
| 输入 | 本轮对话 | 近期多个会话 |
| 工具 | `ReadMemory` `WriteMemory` `UpdateMemory` | 四个（含 `DeleteMemory`） |
| 开关 | `EXTRACT_MEMORY`（设置面板可关） | 见上面的阈值 |

缺了前者，「记住 X」就是句空话：模型只能口头答应，然后等 AutoDream 哪天攒够条件
去会话流水里翻。缺了后者，记忆会越攒越碎。

**提取器的工具集里只有那三个记忆工具**，没有 `DeleteMemory`，更没有任何能碰
业务代码的东西。「这条是不是真没用了」需要全局视野，那是 AutoDream 的活；
而「改不了代码」是**结构性保证**，比在 prompt 里叮嘱「不要修改业务代码」可靠。
有测试锁这一条。

**成本**：每轮多一次 LLM 调用。所以喂给它的是**本轮**的那点东西（用户说了什么、
最后答了什么），不是整个 transcript —— 中间的 Read/Grep 过程对「什么值得长期记住」
毫无价值，喂进去只是白烧 token。prompt 里也写死了：大多数轮次都没有值得记的东西，
**宁可漏掉，也不要写一堆以后没人看的记忆**。

Main Agent 的 system prompt 里明确写着「**不要直接编辑 `.agent/memory/` 下的文件**」——
用通用的 `Write`/`Edit` 改记忆**不会同步索引**，结果是「文件在、索引里没有」的
孤儿记忆。实测里模型确实会自己动手改，所以这条必须明说。
...
它是**防打转的兜底，不是预算**。别拿它当成本控制 —— 写一个 200 行文件是 1 步，
`git status` 也是 1 步，两者成本差两三个量级，共用一个计数器必然一头不够用、
一头管不住。成本看 token 统计（会话列表里就有）。

默认 60：实测「写个程序 + 跑测试 + 调试」这种正常任务要 30-40 步
（有一次 20 步撞上限，光 pytest → 改 → 再 pytest 就占了 17 步）。

撞上限时的**报告**是「调用了哪些工具 + 最后一步在干嘛 + 模型最后说的话」。
只报最后一句发言是不够的：模型经常十几步只调工具、不说话，那时最后一条有文字
的消息可能是第 1 步的开场白 —— 报出来等于没说。网页上还会给一个「接着上次继续」
的按钮。
